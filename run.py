...
def evaluate(model, data_in, data_out, metrics, samples_perc_per_epoch=1, batch_size=500):
    ...
    for batch in generate(batch_size=batch_size,
        ...
        ratings_pred = model(
            ratings_in, calculate_loss=False).cpu().detach().numpy()
...
for epoch in range(args.n_epochs):
    ...
    if args.not_alternating:
        ...
        run(opts=[optimizer_encoder], n_epochs=args.n_enc_epochs,
            dropout_rate=0.5, **learning_kwargs)
        model.update_prior()
        run(opts=[optimizer_decoder], n_epochs=args.n_dec_epochs,
            dropout_rate=0, **learning_kwargs)
...
test_metrics = [{'metric': ndcg, 'k': 100}, {
    'metric': recall, 'k': 20}, {'metric': recall, 'k': 50}]
